...
public class ChessAutomator {
    ...
    private WebDriver createDriver() {
        ...
        if (SeleniumConfig.isRemote()) {
            try {
                System.out.println("[SELENIUM] Using REMOTE WebDriver -> " + SeleniumConfig.remoteUrl());
                return new RemoteWebDriver(new URL(SeleniumConfig.remoteUrl()), options);
            } catch (Exception e) {
                throw new IllegalStateException("Failed to connect to remote Selenium", e);
            }
        } else {
    ...
    private void openAnalysisAndStart() {
        ...
        if (attrs != null && attrs.getPgn() != null) {
            try {
                WebElement textarea = wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("textarea.cc-textarea-component.load-from-pgn-textarea")));
                textarea.clear();
            ...
            try {
                WebElement addButton = driver.findElement(By.xpath("//button[contains(@class, 'cc-button-primary')]//span[text()='Add Game(s)']/ancestor::button"));
            ...
            if (moveNumber >= 0) {
                ...
                WebElement moveList = wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("div.analysis-view-scrollable")));
                try {
                    moveDiv = moveList.findElement(By.cssSelector("div.node.main-line-ply[data-node='" + dataNodeValue + "']"));
        ...
        WebElement moreBtn = wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector("button[data-cy='analysis-secondary-controls-more-button']")));
        moreBtn.click();

        By practiceBtn = By.xpath("//button[.//span[text()='Practice vs Computer']]");
        wait.until(ExpectedConditions.elementToBeClickable(practiceBtn));
        this.initialState = getInitialBoardState();

        WebElement practiceButton = driver.findElement(practiceBtn);
    ...
    public synchronized void loadBotList() throws Exception {
        ...
        try {
            WebElement changeBotButton = wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector("button[aria-label='Change Bot']")));
            ((JavascriptExecutor) driver).executeScript("arguments[0].click();", changeBotButton);

            WebElement scrollContainer = wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("div.bot-selection-scroll")));
            ...
            while (true) {
                ...
                try {
                    for (WebElement tile : tiles) {
                        ...
                        if (seenBots.contains(name)) continue;

                        List<WebElement> lockElements = tile.findElements(By.cssSelector("span.cc-icon-glyph.cc-icon-small.bot-lock"));
                        ...
                        if (isEngine) {
                            WebElement slider = wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("input.slider-input[type='range']")));
                            setSliderValue(slider, Integer.parseInt(slider.getAttribute("min")));
                            String ratingStart = driver.findElement(By.cssSelector("span.selected-bot-rating")).getText().trim().replace("(", "").replace(")", "");
                            setSliderValue(slider, Integer.parseInt(slider.getAttribute("max")));
                            String ratingEnd = driver.findElement(By.cssSelector("span.selected-bot-rating")).getText().trim().replace("(", "").replace(")", "");
                            ...
                            String rating = driver.findElement(By.cssSelector("span.selected-bot-rating")).getText().trim().replace("(", "").replace(")", "");
                            Map<String, Object> botInfo = new HashMap<>();
                            botInfo.put("id", BOTS.size());
                ...
                Boolean isAtBottom = (Boolean) ((JavascriptExecutor) driver).executeScript("let el = arguments[0]; return el.scrollTop + el.offsetHeight >= el.scrollHeight - 5;", scrollContainer);

                if (Boolean.TRUE.equals(isAtBottom)) break;
            }
    ...
    private void setSliderValue(WebElement sliderElement, int targetValue) {
        ...
        int initialRating;
        try {
            initialRating = Integer.parseInt(wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("span.selected-bot-rating"))).getText().replace("(", "").replace(")", ""));
        ...
        wait.until(d -> {
            String newRating = d.findElement(By.cssSelector("span.selected-bot-rating")).getText().replace("(", "").replace(")", "");
        ...
        try {
            ...
            while (true) {
                ...
                for (WebElement tile : tiles) {
                    ...
                    if ((name != null && name.equals(botToSelect.get("name")) && classification != null && classification.equalsIgnoreCase(((String) botToSelect.getOrDefault("classification", "")))) || (isEngine && "engine".equalsIgnoreCase(classification))) {
                        selectedTile = tile;
                        break;
                    }
                }
            ...
            if (selectedTile == null) throw new Exception("Could not find bot in list");
            ...
            WebElement chooseButton = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//button[contains(@class, 'cc-button-primary')]//span[text()='Choose']/ancestor::button")));
    ...
    public Map<String, String> getCurrentBot() {
        try {
            WebElement container = wait.until(ExpectedConditions.presenceOfElementLocated(By.className("player-row-component")));
    ...
    public void undoLastMove() throws Exception {
        try {
            ...
            WebElement undoButton = wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector("button[aria-label='Move Back']")));
            ...
            while (true) {
                ...
                if (!currentState.equals(previousState)) break;
                else {
            ...
            undoButton = wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector("button[aria-label='Move Back']")));
            new Actions(driver).moveToElement(undoButton).click().perform();
    ...
    public Map<String, Object> waitForEngineMove(Map<String, Piece> previousState, int timeoutSeconds) throws TimeoutException {
        ...
        while (System.currentTimeMillis() - start < timeoutMs) {
            ...
            for (String sq : previousState.keySet())
                if (!currentState.containsKey(sq)) removedSquares.add(sq);

            List<String> addedSquares = new ArrayList<>();
            for (String sq : currentState.keySet())
                if (!previousState.containsKey(sq)) addedSquares.add(sq);
            ...
            if (removedSquares.size() == 2 && addedSquares.size() == 2) {
                ...
                boolean hasKing = movedPieces.stream().anyMatch(p -> "k".equals(p.getType()) && side.value().equals(p.getColor()));
                boolean hasRook = movedPieces.stream().anyMatch(p -> "r".equals(p.getType()) && side.value().equals(p.getColor()));
                if (hasKing && hasRook) {
                    String kingFrom = removedSquares.stream().filter(s -> "k".equals(previousState.get(s).getType())).findFirst().orElse(null);
                    String kingTo = addedSquares.stream().filter(s -> "k".equals(currentState.get(s).getType())).findFirst().orElse(null);
            ...
            if (removedSquares.size() == 1 && (addedSquares.size() == 1 || changedSquares.size() == 1)) {
                ...
                System.out.printf("[ENGINE MOVE DETECTED] %s from %s to %s%n", piece.getType().toUpperCase(), squareIndexToAlg(fromSq), squareIndexToAlg(toSq));
                waitingForEngineMove = false;
                // File scrnshot = ((TakesScreenshot) driver).getScreenshotAs(OutputType.FILE);
                // System.out.println(scrnshot.getAbsolutePath());
                Map<String, Object> ret = new HashMap<>();
    ...
    public void completePromotion(String promoteTo) throws Exception {
        if (this.pendingPromotion == null) throw new Exception("No pending promotion to complete.");
        Map<String, String> selectorMap = Map.of("q", ".promotion-piece.wq, .promotion-piece.bq", "r", ".promotion-piece.wr, .promotion-piece.br", "b", ".promotion-piece.wb, .promotion-piece.bb", "n", ".promotion-piece.wn, .promotion-piece.bn");
        if (!selectorMap.containsKey(promoteTo)) throw new IllegalArgumentException("Invalid piece for promotion");
        try {
    ...
    public Map<String, Object> getNextBestMove(String opponentMove) throws Exception {
        if (this.pendingPromotion != null) {
            throw new Exception("[ERROR] Pending promotion detected. Complete it using completePromotion() before proceeding.");
        }
    ...
    public Map<String, Piece> simulateBoardState(Map<String, Piece> state, String move) {
        ...
        if ("k".equals(piece.getType()) && Math.abs(fromAlg.charAt(0) - toAlg.charAt(0)) == 2) {
            String rookFrom = algToSquareIndex(("white".equals(color)) ? (toAlg.charAt(0) == 'g' ? "h1" : "a1") : (toAlg.charAt(0) == 'g' ? "h8" : "a8"));
            String rookTo = algToSquareIndex(("white".equals(color)) ? (toAlg.charAt(0) == 'g' ? "f1" : "d1") : (toAlg.charAt(0) == 'g' ? "f8" : "d8"));
        ...
        if ("p".equals(piece.getType()) && fromAlg.charAt(0) != toAlg.charAt(0) && !newState.containsKey(toSq)) {
            int captureRank = ("white".equals(color)) ? Integer.parseInt(String.valueOf(toAlg.charAt(1))) - 1 : Integer.parseInt(String.valueOf(toAlg.charAt(1))) + 1;
    ...
    public Map<String, Piece> simulateOpponentMove(String move) {
        ...
        if (move.length() < 4) throw new IllegalArgumentException("Moves should be like 'e2e4'");
        ...
        try {
            System.out.printf("[DEBUG] Checking piece at %s (square-%s)%n", fromAlg, fromSq);
    ...
    public String algToSquareIndex(String alg) {
        if (alg == null || alg.length() != 2 || "abcdefgh".indexOf(alg.charAt(0)) < 0 || "12345678".indexOf(alg.charAt(1)) < 0) {
    ...
    public String squareIndexToAlg(String squareId) {
        if (squareId == null || squareId.length() != 2) return "?";
